...
#define AF_LOG_MAX_CALLBACKS 32
...
GlobalVariable struct {
    ...
    int level;
    bool quiet;
    CELlog_callback callbacks[AF_LOG_MAX_CALLBACKS];
} L;
...
Internal void stdout_callback(AFlog_event *event) {
    char buf[16];
...
Internal void file_callback(AFlog_event *event) {
    char buf[64];
...
Internal void event_init(AFlog_event *event, void *data) {
    time_t t = time(NULL);
#if defined(_WIN32)
...
int log_add_callback(AFlog_fn fn, void *data, int level) {
    for (int i = 0; i < AF_LOG_MAX_CALLBACKS; ++i)
    {
        if (!L.callbacks[i].fn)
        {
...
void cel_log(int level, const char *file, int line, const char *fmt, ...) {
    ...
    for (int i = 0; i < AF_LOG_MAX_CALLBACKS && L.callbacks[i].fn; ++i)
    {
